...
- [x] **Cotizadores de servicios (11/08/2026; homologados 13-14/08/2026):** Servicios de Visualización (Renders), Modelado BIM LOD 300 y Planos Ejecutivos Arquitectónicos con mini-cotizadores interactivos. **Los 3 sin descuentos automáticos (se negocian en entrevista) y con categorías unificadas:**
  - **Renders (D5)**: Interior $2,500 · Exterior $3,000/vista · nivel de ambientación ×0.8-×2.5 · flujo 5 pasos (50/50 ↔ 30/40/30 si ≥$15,000) · `POST /cotizar_perspectivas`.
  - **BIM LOD 300**: Casa habitación $90 · Multifamiliar $110 · Comercial $130 por m² · solo Arq/Est (sin MEP) · tarifa mínima $10,000 (oculta) · entregable solo Revit (RVT) · flujo 4 pasos 50/50 · `POST /cotizar_bim`.
  - **Planos Ejecutivos**: Casa habitación $130 · Multifamiliar $160 · Comercial $190 por m² (**sin industrial**) · complejidad del proyecto ×0.85-×1.6 (Simple/Estándar/Complejo) · tarifa mínima $10,000 (oculta) · entregable PDF + DWG · flujo progresivo 6 pasos 30/20/20/20/10 · `POST /cotizar_planos`. Reemplaza a "PLANOS DE ANTEPROYECTO" (eliminado).
  - Los tres: validación de contacto real (correo/teléfono ≥10 dígitos), precio oculto hasta presionar, "+ IVA", solo transferencia, tiempos de entrega de referencia de mercado, botones `.rc-opt` uniformes (seleccionado = borde negro + acento).
- [x] **Fix carrusel landscape (14/08/2026):** carrusel de proyectos se cortaba en celular horizontal; slide vertical compactado (120×200px), sección centrada y título sin padding en `@media (max-height: 520px)`. Verificado en 667×375, 740×360, 844×390, 932×430.
- [x] **Efecto carrusel completo en landscape (14/08/2026, commit `05c57c9`):** en landscape ancho (>768px) las reglas móviles no aplican; `.slide-title` compacto (0.5rem, fondo oscuro redondeado) con efecto naranja `background: var(--accent)` en `.slide.revealed`, e imagen con `grayscale(0.8)/opacity(0.6)/scale(0.9)` y transición completa — idéntico al vertical/laptop.
- [x] **Servicios/Contacto compactos en celular horizontal (14/08/2026, commit `d077a87`):** la sección no cabía en landscape (Samsung A12, alto efectivo ~300px con barra del navegador). `.services-row` en 2 columnas (menú | visual) + cotizadores compactos (visual 210px, panel min-height 200px) en `@media (max-height: 520px)`. Verificado en 800×360, 800×300, 667×375, 568×320 con los 4 cotizadores: contacto cabe, sin solapamientos ni scroll horizontal.
- [x] **Menú de navegación web (25/08/2026, commit `d054c08`):** hamburguesa minimalista fija arriba-derecha (alineada al eje del botón Cotizador SOMA), panel desplegable sutil (fondo 18% + blur) con anclas a las 5 secciones (`#inicio/#portafolio/#trayectoria/#filosofia/#servicios`). Scroll suave compatible con scroll-snap; cierra con Esc/clic fuera.
- [x] **GitHub (14/08/2026):** push de cotizadores homologados (`ce7b91d`), fix carrusel (`bef0557`), efecto carrusel (`05c57c9`) y servicios landscape (`d077a87`). Diagnóstico del "constructor" matutino: workers OK; probable límite de 750 h/mes de Render Free.
