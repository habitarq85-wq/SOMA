...
- **Mini-cotizadores de servicios (11/08/2026; homologados 13/08/2026):** Al seleccionar Visualización (Renders), Modelado BIM LOD 300 o Planos Ejecutivos en Servicios se muestra un cotizador interactivo con correo/WhatsApp a `habitarq85@gmail.com` vía Brevo. **Los 3 comparten flujos y categorías unificadas, sin descuentos automáticos (se negocian en entrevista):**
  - **Renders (Visualización):** Interior $2,500 · Exterior $3,000 por render. Nivel de ambientación ×0.8 (Básico) / ×1.0 (Medio) / ×2.5 (Alto). Flujo de 5 pasos: 1. Contratación → 50% anticipo (30% si ≥$15,000); 2. Primera entrega → propuesta de arranque; 3. Revisión → ajuste (40% si ≥$15,000); 4. Última revisión → detalles finales; 5. Entrega final → 50% restante (30% si ≥$15,000). Endpoint `POST /cotizar_perspectivas`.
  - **BIM LOD 300:** Casa habitación $90 · Multifamiliar $110 · Comercial $130 por m² (categorías renombradas a las de Planos). Solo Arquitectura y Estructura (mismo precio, **sin MEP**). Tarifa mínima **$10,000 MXN** (oculta, aplica en cálculo). Entregable: modelo 3D arquitectónico en **Revit (RVT)** con especificación general; alcance se confirma en la entrevista. Flujo de 4 pasos 50/50: 1. Contratación → 50% anticipo; 2. Primera entrega → modelo base 3D; 3. Revisión → ajuste (1 ronda); 4. Entrega final → RVT completo. Endpoint `POST /cotizar_bim`.
  - **Planos Ejecutivos:** Casa habitación $130 · Multifamiliar $160 · Comercial $190 por m² (**sin industrial/naves industriales**). Complejidad del proyecto ×0.85 (Simple) / ×1.0 (Estándar) / ×1.6 (Complejo). Tarifa mínima **$10,000 MXN** (oculta, aplica en cálculo). Entregable: **PDF + DWG** (planos arquitectónicos, ejecutivos generales y ejecutivos de detalle). Flujo progresivo de 6 pasos 30/20/20/20/10: 1. Contratación → 30% anticipo; 2. Primera entrega → arquitectónicos (20%); 3. Segunda entrega → ajuste + ejecutivos generales (20%); 4. Tercera entrega → ajuste + ejecutivos de detalle (20%); 5. Entrega preliminar → ajuste final (incluido); 6. Entrega final → documentación completa (10%). Endpoint `POST /cotizar_planos`. **Reemplaza al servicio "PLANOS DE ANTEPROYECTO" (eliminado de la lista).**
  - **Los tres:** validación de contacto real (correo o teléfono ≥10 dígitos), precio oculto hasta presionar "VER COTIZACIÓN", total muestra "+ IVA", pago dinámico (ver flujos arriba), solo transferencia. Los tiempos mostrados son **referencia de mercado**; el plazo definitivo se compromete en la **entrevista de alcance**.
  ...
  - **Botones `.rc-opt` unificados (13/08/2026):** borde `transparent` en todos; seleccionado `.rc-opt.on` = `background: var(--accent); border-color: #000; color:#fff; font-weight:700`; no seleccionados con fondo visible `rgba(168,62,24,0.55)`. Tipografía 0.5rem desktop / 0.42rem móvil. Sin `backdrop-filter` en móvil (lentitud Android).
  - **Fix carrusel landscape (14/08/2026):** el carrusel de proyectos se cortaba en celular horizontal; se compactó el slide vertical de la sección 2 (120×200px), se centró la sección y se anuló el padding del título en `@media (max-height: 520px)`. Verificado con Puppeteer en 667×375, 740×360, 844×390, 932×430. **Efecto completo (14/08/2026, commit `05c57c9`):** en landscape ancho (>768px) las reglas de `max-width:768px` no aplican, así que `.slide-title` se compactó (0.5rem, fondo oscuro redondeado abajo-izquierda) con **efecto naranja** `background: var(--accent)` en `.slide.revealed`, y la imagen parte de `grayscale(0.8)/opacity(0.6)/scale(0.9)` con transición completa — idéntico al vertical/laptop.
  - **Servicios/Contacto compactos en celular horizontal (14/08/2026, commit `d077a87`):** la sección no cabía completa en landscape (Samsung A12, alto efectivo ~300px con barra del navegador). `.services-row` ahora usa **2 columnas** (menú | visual) en `@media (max-height: 520px)` ahorrando ~44px de alto, y los **cotizadores se compactaron** (`.services-visual` 210px, imagen 150px, panel `min-height` 200px, paddings/fuentes/gaps reducidos) para que quepan dentro del visual sin montar información. Verificado en 800×360, 800×300, 667×375 y 568×320 con los 4 cotizadores abiertos: contacto cabe, panel dentro del visual sin scroll interno, sin solapamientos, sin scroll horizontal.
...
- [x] **Cotizadores homologados a flujos únicos y sin descuentos (13-14/08/2026):** renders 5 pasos (50/50↔30/40/30 si ≥$15,000), BIM 4 pasos 50/50, Planos 6 pasos progresivo 30/20/20/20/10; categorías unificadas CASA HABITACIÓN/MULTIFAMILIAR/COMERCIAL; tarifa mínima $10,000 (oculta); botones `.rc-opt` uniformes con borde negro en selección; commit `ce7b91d` en GitHub.
- [x] **Fix carrusel de proyectos en celular horizontal (14/08/2026):** slide vertical de la sección 2 compactado (120×200px), sección centrada y título sin padding en `@media (max-height: 520px)`; commit `bef0557`.
- [x] **Efecto carrusel completo en landscape (14/08/2026):** título compacto con efecto naranja + imagen con `grayscale/opacity/scale` y transición en `@media (max-height: 520px)` — idéntico al vertical/laptop; commit `05c57c9`.
- [x] **Servicios/Contacto compactos en celular horizontal (14/08/2026):** `.services-row` en 2 columnas (menú | visual) + cotizadores compactos (visual 210px, panel 200px) para que quepan sin montar información; verificado en 800×360, 800×300, 667×375, 568×320 con los 4 cotizadores; commit `d077a87`.
...
## 📅 ÚLTIMA ACTUALIZACIÓN: 14/08/2026 (COTIZADORES HOMOLOGADOS + FIX CARRUSEL LANDSCAPE + SERVICIOS/CONTACTO COMPACTOS EN CELULAR HORIZONTAL + PUSH GITHUB)
...
## 📅 SESIÓN 11/08/2026: COTIZADOR PLANOS EJECUTIVOS ARQUITECTÓNICOS (nuevo)

### ✅ COMPLETADO

- **[NUEVO] Cotizador Planos Ejecutivos en Servicios:** tercer mini-cotizador al seleccionar "PLANOS EJECUTIVOS" (index 3, reemplaza a "PLANOS DE ANTEPROYECTO" que se eliminó de la lista). Precios por m² (rango bajo del mercado, validado 2026): Vivienda $130 · Residencia $160 · Comercial $190. **Sin industrial/naves industriales** (decisión de Juan). Complejidad del proyecto (Simple/Estándar/Complejo) ×0.85/×1.0/×1.6. Descuentos >300 m² −5%, >600 m² −10%. Tarifa mínima $10,000 MXN. Entregable: PDF + DWG. Endpoint `POST /cotizar_planos` (público) con correo a `habitarq85@gmail.com`.
- **Fuentes de validación 2026:** Arqbeat, Ernesto Resendiz, Arqzon, PE BIM, Cronoshare Mérida, Habitissimo Mérida (planos básicos $8,000–$25,000; ejecutivo completo $200–$750/m²; Mérida $11,000–$35,000/proyecto).

## 📅 SESIÓN 11/08/2026: COTIZADORES D5 + BIM LOD 300 (actividades y precios definidos)

### ✅ COMPLETADO

- **[NUEVO] Cotizador BIM LOD 300 en Servicios:** segundo mini-cotizador al seleccionar "MODELADO BIM LOD 300". Precios por m² (rango bajo del mercado): Vivienda $90 · Residencial $110 · Comercial $130 · Industrial $150. Solo Arquitectura y Estructura (mismo precio, sin MEP). Descuentos >500 m² −5%, >1,000 m² −10%. Tarifa mínima $12,000 MXN. Entregable: solo Revit (RVT). Endpoint `POST /cotizar_bim` (público) con correo a `habitarq85@gmail.com`.
- **[NUEVO] Cotizador D5 (Visualización) terminado:** botón "VER COTIZACIÓN" con validación de contacto real, precio oculto hasta presionar, `POST /cotizar_perspectivas`.
- **[NUEVO] Tiempos de entrega con referencia de mercado 2026:** D5 3-5 días/vista, paquetes 1-3 semanas; BIM 2-6 semanas según m². El cotizador es referencia; el plazo definitivo se compromete en la entrevista de alcance.
- **[NUEVO] Esquema de pago dinámico:** 50/50 si total < $15,000; 30/40/30 si ≥ $15,000. Solo transferencia.
- **[NUEVO] IVA explícito:** los totales muestran "+ IVA" (se eliminó el ambiguo "No incluye IVA").
- **[NUEVO] Referencias de mercado documentadas:** Budgeto MX (LOD 300 $90-150 MXN/m²), Carnet 3D (renders MX 2026), myarchitectai (internacional por imagen), ENGINYRING (scan-to-BIM 2026), Arrival 3D (2-4 semanas), GCC (villas 4-9 días).

### 📁 ARCHIVOS CREADOS/MODIFICADOS

| Archivo | Descripción |
|---------|-------------|
| `web/Pagina Web 6.html` | **Modificado** — Cotizador BIM LOD 300 (HTML+JS), `changeService` index 2, tiempos de mercado, pago dinámico, + IVA, entregable solo RVT |
| `web/Pagina Web 6.html` | **Modificado** — Cotizador Planos Ejecutivos (HTML+JS), `changeService` index 3 (reemplaza "PLANOS DE ANTEPROYECTO"), tiempos de mercado, pago dinámico, + IVA, entregables PDF+DWG |
| `backend/server.py` | **Modificado** — Endpoint `POST /cotizar_bim` + `public_paths` |
| `backend/server.py` | **Modificado** — Endpoint `POST /cotizar_planos` + `public_paths` |

### 📋 PENDIENTES

1. Continuar con la web: revisar cotizadores en vivo y pulir copy de Servicios
2. Vincular estaciones 4+ (Conceptualización, Modelado, Visualización) con datos de la BD
3. Crear tabla `algoritmo_contenido` para outputs de cada estación
4. Lead magnet — decidir ubicación en página web
